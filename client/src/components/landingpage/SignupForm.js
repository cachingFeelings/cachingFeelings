...
const SignupForm = () => {
  ...
  const handleSignUp = async (e) => {
    ...
    try {
      const res = await fetch("https://caching-feelings-server.onrender.com/user/create_user", {
        method: "POST",
        headers: {
      ...
      if (token) {
        if (token === "undefined") {
          ...
          try {
            const res2 = await fetch("https://caching-feelings-server.onrender.com/user/login", {
              method: "POST",
              headers: {
                'Content-Type': 'application/json'
              },
